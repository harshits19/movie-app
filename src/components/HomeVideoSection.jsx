    ...
    <div className="relative">
      <div className="absolute inset-0 contents">
        <div className="pointer-events-none relative h-full min-h-[20rem] overflow-hidden sm:aspect-video">
          {mainMovie?.backdrop_path && <img className="h-full min-h-[20rem] object-cover sm:aspect-video" src={"https://image.tmdb.org/t/p/original/" + mainMovie?.backdrop_path} />}
      ...
      <div className="absolute left-0 top-0 h-full w-full pt-28 text-white md:pt-40 lg:pt-60 xl:pt-80 ">
        <div className="w-full pl-8 pr-4 sm:w-1/2 md:pl-8 md:pr-0 lg:pl-16 xl:w-1/3">
          <div className="text-xl font-bold md:text-3xl lg:text-5xl">{mainMovie?.title || mainMovie?.name || mainMovie?.original_name}</div>
          <div className="pt-4 text-xs font-semibold md:text-sm">{mainMovie?.overview?.slice(0, 150) + "..."}</div>
          <div className="mt-4 flex">
            <Link to={"" + mainMovie?.id} state={{ item: mainMovie, type: "tv" }} className="contents">
            ...
            <Link to={"" + mainMovie?.id} state={{ item: mainMovie, type: "tv" }} className="contents">
              <div className="ml-2 flex cursor-pointer items-center gap-x-1 rounded bg-[#6d6d6eb3] px-2 py-1 text-xs font-semibold text-white hover:bg-[#6d6d6e66] sm:text-sm md:gap-x-[0.75rem] md:px-4 lg:px-6 lg:py-1.5 lg:text-base">
                <InfoBtn className="h-5 w-5 fill-white md:h-7 md:w-7" title="Info" />
                More Info
              </div>
            </Link>
          </div>
