  ...
  return (
    <div className="relative">
      <div className="mt-20 flex h-full w-full items-center gap-x-4 md:mt-32">
        <h1 className="ml-8 text-2xl font-bold text-white sm:ml-12 md:text-3xl lg:text-5xl">Movies</h1>
      ...
      {status === "success" && (
        <div className="categoryStripes relative pt-8">
          ...
          <CategoryStripes data={trending} title="Trending Movies" type="movie" />
          <CategoryStripes data={topRated} title="Top Rated Movies" type="movie" />
        </div>
      )}
      {status === "loading" && (
      ...
      {status === "failed" && (
        <div className="relative ml-12 mt-8">
          <h1 className="text-center text-3xl text-white">Error, cannot find movies related to selected genre.</h1>
        </div>
      )}
      <Outlet />
    </div>
